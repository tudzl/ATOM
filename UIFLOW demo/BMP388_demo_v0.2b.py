...
class BMP3XX:
    ...
    def __init__(self):
        ...
        if _CHIP_ID != chip_id:
            print('BMP388 chip id error')
            #raise RuntimeError("Failed to find BMP3XX! Chip ID 0x%x" % chip_id) #TypeError: can't convert NoneType to int
  ...
  try:
    ...
    for i in range(0, Dev_cnt, 1):
     ...
     if (119==addrList[i]): #BMP388
        ...
        print(i2c0.read_reg(0x00, 1)) #OK! 0x50  b'P'
        #print(' chip ID is:'+ str(i2c0.read_u8(_REGISTER_CHIPID)))
        wait_ms(500)
